...
public class PlayerMoving : MonoBehaviour 
{
    ...
    private void Update()
    {
        float h = Input.GetAxis("Horizontal") * Time.deltaTime * speed; //left and right movement
        float v = Input.GetAxis("Vertical") * Time.deltaTime * speed; //up and down movement

        float newXpos = transform.position.x + h; // update h position;
        float newYpos = transform.position.y + v; // update v position;
